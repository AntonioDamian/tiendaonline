    ...
    public class ArticuloADO:ADO
    {
...

        {
            try
            {
               ...
               var response = client.PostAsync("api/articulos", new StringContent(new JavaScriptSerializer().Serialize(articulo), Encoding.UTF8, "application/json")).Result;
                if (response.IsSuccessStatusCode)
                {
                    return true;
                }
        ...
        public bool ActualizarArticulo(Articulo articulo)
        {
            try
            {
               // HttpResponseMessage response = client.PutAsJsonAsync("api/articulos/" + articulo.ArticuloID, articulo).Result;
                var response = client.PutAsync("api/articulos", new StringContent(new JavaScriptSerializer().Serialize(articulo.ArticuloID), Encoding.UTF8, "application/json")).Result;
